...
//             })
            this.plusPostData.progress = 0
            uni.navigateTo({
            	url:'/pages/plus/fail?msg=' + ret.message
            })
    ...
    async onLoad() {
      ...
      if(this.userInfo.share_level != 1){
        uni.redirectTo({
        	url:'/pages/plus/fail?type=1&msg=' + '您还未通过评测权限申请！'
        })
      }
      ...
      if(ret.code == 0){
        ...
        if(count <= 0){
          uni.redirectTo({
          	url:'/pages/plus/fail?type=1&msg=' + '您还未购买过商城商品，不能进行评测！'
          })
        }
      }else {
        uni.redirectTo({
        	url:'/pages/plus/fail?type=1&msg=' + '网络发生错误，请稍后重试!'
        })
      }
