    ...
    <view v-if="orderListData.count" class="">
      ...
      <view class="uni-bg-white uni-common-mt" v-for="(order,index) in orderListData.list" :key="index">
        ...
        <view class="uni-border-top  uni-common-pb" @tap="goToDetail(order)">
        	...
        	<view class="uni-flex uni-common-pl uni-common-pr uni-common-pt" v-for="(item,index1) in order.goods_items" :key="index1">
        ...
        <view class="uni-flex uni-border-top uni-common-pa uni-bg-white">
          ...
          <view class="" v-if="order.status == 9">
          	<text class="order-btn ">申请售后</text>
          </view>
        </view>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      goToDetail(order){
        uni.navigateTo({
        	url:'/pages/user/orderDetail?id=' + order.id
        })
      },
      goCancel(order){
        uni.showModal({
